...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        ExecutorService uiExecutor = Executors.newSingleThreadScheduledExecutor();
        ...
        uiExecutor.execute(() -> {
            while (!allDone) {
                try {
                    Thread.sleep(1000);
                    System.out.println("Downloading files... (" + (System.currentTimeMillis() - startTime) / 1000 + "s)");
                } catch (InterruptedException e) {
                    Thread.currentThread().interrupt();
                }
            }System.out.println("Downloading Complite");
        });
        ...
        for (int i = 1; i <= totalFiles; i++) {
            ...
            downloadExecutor.execute(() -> {
                ...
                int duration = ThreadLocalRandom.current().nextInt(1, 4); // 1-3 detik
                try {
